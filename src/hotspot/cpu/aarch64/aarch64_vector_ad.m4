...
instruct vmaskcmp_sve_imm5_I(pReg dst, vReg src, immI imm_compared, immI cond, rFlagsReg cr) %{
  ...
  effect(KILL cr);
  format %{ "vmaskcmp_sve_imm5_I $dst, $src, $imm_compared, $cond\t# KILL cr" %}
  ins_encode %{
    BasicType bt = Matcher::vector_element_basic_type(this);
    uint length_in_bytes = Matcher::vector_length_in_bytes(this);
    assert(length_in_bytes == MaxVectorSize, "invalid vector length");
...
instruct vmaskcmp_sve_imm5_L(pReg dst, vReg src, immL imm_compared, immI cond, rFlagsReg cr) %{
  predicate(UseSVE > 0 &&
            (n->in(1)->in(2)->in(1)->get_long() >= -16 && n->in(1)->in(2)->in(1)->get_long() <= 15));
  match(Set dst (VectorMaskCmp (Binary src (ReplicateL imm_compared)) cond));
  format %{ "vmaskcmp_sve_imm5_L $dst, $src, $imm_compared, $cond\t# KILL cr" %}
  ins_encode %{
    uint length_in_bytes = Matcher::vector_length_in_bytes(this);
    assert(length_in_bytes == MaxVectorSize, "invalid vector length");
    __ sve_compare_imm($dst$$PRegister, T_LONG, ptrue, $src$$FloatRegister,
                       (int)($imm_compared$$constant), (int)($cond$$constant));
  %}
  ins_pipe(pipe_slow);
%}
...
instruct vmaskcmp_sve_imm7_I(pReg dst, vReg src, immI imm_compared, immI cond, rFlagsReg cr) %{
  ...
  match(Set dst (VectorMaskCmp (Binary src (ReplicateI imm_compared)) cond));
  effect(KILL cr);
  format %{ "vmaskcmp_sve_imm7_I $dst, $src, $imm_compared, $cond\t# KILL cr" %}
  ins_encode %{
    BasicType bt = Matcher::vector_element_basic_type(this);
    uint length_in_bytes = Matcher::vector_length_in_bytes(this);
    assert(length_in_bytes == MaxVectorSize, "invalid vector length");
...
instruct vmaskcmp_sve_imm7_L(pReg dst, vReg src, immL imm_compared, immI cond, rFlagsReg cr) %{
  predicate(UseSVE > 0 &&
            (n->in(1)->in(2)->in(1)->get_long() >= 0 && n->in(1)->in(2)->in(1)->get_long() <= 127) &&
            (n->in(2)->get_int() != BoolTest::eq && n->in(2)->get_int() != BoolTest::ne));
  match(Set dst (VectorMaskCmp (Binary src (ReplicateL imm_compared)) cond));
  effect(KILL cr);
  format %{ "vmaskcmp_sve_imm7_L $dst, $src, $imm_compared, $cond\t# KILL cr" %}
  ins_encode %{
    uint length_in_bytes = Matcher::vector_length_in_bytes(this);
    assert(length_in_bytes == MaxVectorSize, "invalid vector length");
    __ sve_compare_imm($dst$$PRegister, T_LONG, ptrue, $src$$FloatRegister,
